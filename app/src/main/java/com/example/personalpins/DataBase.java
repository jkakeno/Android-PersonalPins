...
public class DataBase extends SQLiteOpenHelper {
    ...
    * https://stackoverflow.com/questions/21432556/android-java-lang-illegalstateexception-couldnt-read-row-0-col-0-from-cursorw*/
    ...
    public ArrayList<Pin> getPinList(String query) {
        /*Make an empty list.*/
        ArrayList<Pin> pinList = new ArrayList<>();
        if(!query.isEmpty()) {
       /*Query the database.*/
        SQLiteDatabase db = this.getReadableDatabase();
        db.beginTransaction();
        Cursor cursorTag = db.rawQuery("SELECT * FROM " + TABLE_TAG + " WHERE " + COL_TAG + " LIKE '%" + query + "%' ", null);
            if (cursorTag.moveToFirst()) {
                do {
                    long pinId = cursorTag.getLong(cursorTag.getColumnIndex(COL_TAG_FOREIGN_KEY));
                    Cursor cursorPin = db.rawQuery("SELECT * FROM " + TABLE_PIN + " WHERE " + BaseColumns._ID + " = " + pinId + " ORDER BY " + BaseColumns._ID + " ASC", null);
                    if (cursorPin.moveToFirst()) {
                        do {
                            String pinTitle = cursorPin.getString(cursorPin.getColumnIndex(COL_PIN_TITLE));
                            String pinImage = cursorPin.getString(cursorPin.getColumnIndex(COL_PIN_IMAGE));
                            String pinVideo = cursorPin.getString(cursorPin.getColumnIndex(COL_PIN_VIDEO));
